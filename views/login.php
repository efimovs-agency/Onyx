...
<head>
    ...
    <style>
        ...
        /* Стили для плавного переключения форм авторизации */
        .auth-form-wrapper { display: none; }
        .auth-form-wrapper.active { display: block; animation: fadeInForm 0.4s ease forwards; }
...
<?php if (!isset($_SESSION['user_id'])): ?>
    <?php 
        // Жестко определяем состояние формы из контроллера, исключая баги с strpos
        $activeForm = $activeForm ?? ($_SERVER['REQUEST_URI'] === '/login' ? 'login' : 'register');
        $isLoginState = ($activeForm === 'login');
    ?>
    <div class="auth-layout">
        <div class="card auth-card">
            
            <div id="registerWrapper" class="auth-form-wrapper <?= !$isLoginState ? 'active' : '' ?>">
                <h2><?= __('registration') ?? 'Регистрация' ?></h2>
                
                <?php if (isset($error) && !$isLoginState): ?>
                    <?php if (strpos($error, 'успешна') !== false): ?>
                ...
                <form action="/" method="POST" novalidate autocomplete="off">
                    ...
                    <input type="email" name="reg_email" data-secure-id="email" value="<?= htmlspecialchars($_POST['reg_email'] ?? '') ?>" placeholder="<?= __('email_placeholder') ?? 'Email' ?>" required autocomplete="off" spellcheck="false" data-lpignore="true">
                    <input type="text" name="reg_login" data-secure-id="login" value="<?= htmlspecialchars($_POST['reg_login'] ?? '') ?>" placeholder="<?= __('login_placeholder') ?? 'Логин' ?>" required autocomplete="off" spellcheck="false" data-lpignore="true">
                    <input type="password" name="reg_password" data-secure-id="password" class="input-password-secure" placeholder="<?= __('password_placeholder') ?? 'Пароль' ?>" required autocomplete="off" spellcheck="false" data-lpignore="true">
                ...
                <div class="auth-footer" style="display: flex; justify-content: space-between; align-items: center; margin-top: 24px;">
                    <a href="javascript:void(0)" onclick="toggleAuthMode('login')" class="link-muted"><?= __('already_have_account') ?? 'Уже есть аккаунт?' ?></a>
            ...
            <div id="loginWrapper" class="auth-form-wrapper <?= $isLoginState ? 'active' : '' ?>">
                <h2><?= __('login_title') ?? 'Вход' ?></h2>
                ...
                <form action="/login" method="POST" novalidate autocomplete="off">
                    ...
                    <input type="password" name="password" data-secure-id="password" class="input-password-secure" placeholder="<?= __('password_placeholder') ?? 'Пароль' ?>" required autocomplete="off" spellcheck="false" data-lpignore="true">
                    <button type="submit"><?= __('login_btn') ?? 'Войти' ?></button>
                </form>
                
                <div class="auth-footer" style="display: flex; justify-content: space-between; align-items: center; margin-top: 24px;">
                    <a href="javascript:void(0)" onclick="toggleAuthMode('register')" class="link-muted"><?= __('no_account') ?? 'Нет аккаунта?' ?></a>
            ...
            <script>
                // Скрипт для мгновенного переключения между логином и регистрацией
...
<?php else: ?>
    ...
    <div class="settings-modal" id="settingsContent" onclick="event.stopPropagation()">
        <div class="settings-header">
            ...
            <button type="button" aria-label="Закрыть настройки" onclick="closeSettingsModal()" style="background: transparent; border: none; cursor: pointer; padding: 8px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: inherit; opacity: 0.7; transition: opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.7'">
